<div align="center">
  ...
  <p>提示词、待办、随笔记、链接、录音与本机 AI 提醒，始终贴顶待命。</p>
  <p>
...
| 页面 | 解决什么问题 |
| --- | --- |
| **首页** | 当前窗口、镜子、快速录音、随笔记、提示词快捷入口、汽水音乐和番茄钟集中在一个 Bento 工作台 |
| **待办** | 四个可改名的工作流，截止日期可逐月切换并自然跨年，按截止时间排序，并在到期前一小时提醒 |
| **提示词** | 本地保存、搜索、收藏和复用提示词，并可用已配置的模型自动生成标题与标签 |
...
- **AI 整理需授权**：提示词先在本机保存；只有用户明确开启后，正文才会发送至其配置的模型服务生成标题和标签。
